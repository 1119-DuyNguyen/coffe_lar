...
@if (!empty($formElement))
    <div class="form-group">
        <label
            class="lowercase-and-capitalize-first-letter">{{ $formElement['label'] ?? '' }}{!! empty($formElement['optional'])
                ? '<sup class="text-danger-600 dark:text-danger-400 font-medium">*</sup>'
                : '' !!}</label>
        @switch($formElement['type'])
            @case('select')
                <select class="form-control" name="{{ $formElement['name'] }}"
                    {{ isset($formElement['disabled']) ? 'disabled' : '' }}>
                    @foreach ($formElement['optionValues'] as $option)
                        @php
                            $optionValue = $option[$formElement['optionKey']];

                        @endphp
                        <option value="{{ $optionValue }}" {{ $optionValue == $formElement['value'] ? 'selected' : '' }}>
                    ...
                    @foreach ($formElement['optionValues'] as $option)
                        @php
                            ...
                            if (empty($formElement['value'])) {
                                $formElement['value'] = [];
                            } elseif (!is_array($formElement['value'])) {
            ...
            @case('textfield')
                <textarea name="{{ $formElement['name'] }}" class="form-control {{ $formElement['class'] ?? '' }}">{!! $formElement['value'] !!}</textarea>
            @break

            @case('disabled')
                <input type="text" class="form-control {{ $formElement['class'] ?? '' }}" name="{{ $formElement['name'] }}"
                    value="{{ $formElement['value'] }}" disabled>
            @break

            @default
                <input type="{{ $formElement['type'] }}" class="form-control {{ $formElement['class'] ?? '' }}"
                    name="{{ $formElement['name'] }}" value="{{ $formElement['value'] }}">
        @endswitch
